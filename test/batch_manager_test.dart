...
void main() {
  test("batch Manager", () async {
    called = false;
    HitCacheManager fsCacheHitTest = HitCacheManager();

    TrackingManagerConfig configTrackingTest = TrackingManagerConfig(
        batchIntervals: 20, poolMaxSize: 50, batchStrategy: BatchCachingStrategy.BATCH_CONTINUOUS_CACHING);

    FlagshipPoolQueue testPool = FlagshipPoolQueue(configTrackingTest.poolMaxSize);
    for (int i = 0; i < 50; i++) {
    ...
    // Create batch object
    BatchManager testBatch = BatchManager(testPool, sendBatchTest, configTrackingTest, fsCacheHitTest);
    // trigger the batch
  ...
  test("batch Manager with timer", () async {
    ...
    TrackingManagerConfig configTrackingTest = TrackingManagerConfig(
        batchIntervals: 2, poolMaxSize: 50, batchStrategy: BatchCachingStrategy.BATCH_CONTINUOUS_CACHING);
  ...
  test("batch Manager with limite size", () async {
    ...
    TrackingManagerConfig configTrackingTest = TrackingManagerConfig(
        batchIntervals: 60, poolMaxSize: 30, batchStrategy: BatchCachingStrategy.BATCH_CONTINUOUS_CACHING);
